...
async function registerPinkDriver(event) {
    ...
    if (!name || !phone || !email || !age || !experience || !city || !state ||
        ...
        pinkDriverShowModal('All fields are required.', false);
        return;
    }
...
function pinkDriverShowModal(message, isSuccess = true) {
    ...
    const modal = document.getElementById('pink-driver-successModal');
    const messageElement = document.getElementById('pink-driver-modalMessage');
    const iconElement = document.getElementById('pink-driver-modalIcon');
    ...
    console.log("Before modal display");
    ...
    document.querySelector('.pink-driver-close').onclick = function () {
...
document.querySelector('.cab-taxi-driver-forms-registration-form').addEventListener('submit', registerPinkDriver);
